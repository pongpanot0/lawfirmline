      ...
      <section className={group} aria-labelledby="case-people-heading">
        ...
        <div className="space-y-3 text-sm sm:col-span-2">
          {value.customers.map((customer, index) => {
            ...
            return <div key={`${customer.customerId}-${index}`} className="grid min-w-0 gap-2 sm:grid-cols-[minmax(0,1fr)_auto_auto] sm:items-end">
              <div className="min-w-0"><CustomerSelect id={`overview-customer-${index}`} label={`ผู้ว่าจ้างรายที่ ${index + 1}`} value={customer.customerId} clients={clients} onChange={customerId => onChange({ ...value, customers: value.customers.map((row, i) => i === index ? { ...row, customerId, contactId: '' } : row) })} onCreated={onClientCreated} />
                {contacts.length > 0 && <label className="mt-2 block text-xs text-muted-foreground">ผู้ติดต่อ<select aria-label={`ผู้ติดต่อผู้ว่าจ้างรายที่ ${index + 1}`} value={customer.contactId} onChange={e => onChange({ ...value, customers: value.customers.map((row, i) => i === index ? { ...row, contactId: e.target.value } : row) })} className={input}><option value="">ยังไม่ระบุ</option>{contacts.map(contact => <option key={contact.id} value={contact.id}>{contact.name}</option>)}</select></label>}
              </div>
              {value.customers.length > 1 && <label className="text-xs text-muted-foreground">สัดส่วน (%)<input type="number" min="0" max="100" value={customer.sharePercent} onChange={e => onChange({ ...value, customers: value.customers.map((row, i) => i === index ? { ...row, sharePercent: e.target.value } : row) })} className={input} /></label>}
