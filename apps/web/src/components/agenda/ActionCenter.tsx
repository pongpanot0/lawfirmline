  ...
  return <Card>
    ...
    <CardContent className="space-y-3">
      {error ? <div role="alert"><p>{th ? 'โหลดรายการไม่สำเร็จ' : 'Could not load actions'}</p><Button variant="outline" onClick={() => setAttempt(a => a + 1)}>{th ? 'ลองอีกครั้ง' : 'Retry'}</Button></div> : !queue ? <p role="status">{th ? 'กำลังโหลด…' : 'Loading…'}</p> : <>
        ...
        <ul className="divide-y">{items.map(item => <li key={item.id}><Link href={item.url} className="flex min-h-16 items-center gap-3 rounded-md py-3 focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary hover:bg-muted/40"><div className="min-w-0 flex-1"><p className="text-xs font-medium text-primary">{labels[item.kind]?.[th ? 0 : 1]}</p><p className="break-words font-medium">{item.title || (th ? 'เรื่องรับเข้า' : 'Intake')}</p><p className="text-xs text-muted-foreground">{[item.caseRef, item.owner ?? (th ? 'ยังไม่มอบหมาย' : 'Unassigned'), item.dueAt ? new Date(item.dueAt).toLocaleDateString(th ? 'th-TH' : 'en-GB', { timeZone: 'Asia/Bangkok', day: 'numeric', month: 'short', year: 'numeric' }) : (th ? 'ยังไม่กำหนดวัน' : 'No date set')].filter(Boolean).join(' · ')}</p>{item.detail && <p className="mt-1 line-clamp-2 text-xs text-muted-foreground">{item.detail}</p>}</div><ArrowRight className="size-4 shrink-0" aria-hidden /></Link></li>)}</ul>
