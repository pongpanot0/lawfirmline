...
export async function workroomRequest<T>(token: string, staff: boolean, path: string, body?: object | FormData, file = false): Promise<T> {
  ...
  if (!response.ok) { const data = await response.json().catch(() => ({})); const message = Array.isArray(data.message) ? data.message.join(', ') : data.message ?? `Request failed (${response.status})`; if (body) publishActionFeedback('error', `ทำรายการไม่สำเร็จ: ${message}`); throw new Error(message); }
